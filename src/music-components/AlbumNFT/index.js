...
const AlbumNFT = ({ type }) => {
    ...
    //                 <p onClick={async ()=>{
    //                      await request(`/api/album/delete/${id}`, {}, {}, "GET")

    //                 }} className="text-red-800"> Delete   </p>
    //             </a>
    //           ),
    //         },
    //       ]}
                                            ...
                                            items={[
                                                ...
                                                },
                                                {
                                                    label: (
                                                        <a
                                                            onClick={async () => {
                                                                const result=await request(`/api/album/delete/${album.id}`, {}, {}, "GET")
                                                                if(result){
                                                                    toast.success("Delete album success")
                                                                }
                                                                else{
                                                                    toast.success("Delete album failed")
                                                                }
                                                                setListAlbum(listAlbum.filter(item => item.id !== album.id))

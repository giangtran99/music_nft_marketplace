...
const NFTInfo = (props) => {
    ...
    const [priceHistorys, setPriceHistory] = React.useState([])
    const { id } = useParams();
    ...
    const getPriceHistory = async () => {
        ...
        return result4.data.transactionLogs.map(item=>{
            return {
                "ethPrice":formatPrice(item.ethPrice),
                "created_at":moment(item.created_at*1000).format("DD-MM-YYYY")
            }
        })
    }
    ...
    const SelectFilter = ({ onChange }) => {
        ...
        return (
            <>
                ...
                }} showSearch style={{ width: "100%" }} defaultValue={data[0]?.key}>
                    {data.map((item) => (
            ...
            <section className="text-gray-700 body-font overflow-hidden">
                    ...
                    <div className="lg:w-3/5 m-auto flex flex-wrap">
                        <div key={""} className="bg-white-900 shadow-2xl rounded p-3">
                            <div className="group relative">
                                <img className="m-auto w-72 block rounded" src={nftInfo.coverPhoto ? `${process.env.REACT_APP_IPFS_URL}:${process.env.REACT_APP_IPFS_GATEWAY_PORT}/ipfs/${nftInfo.coverPhoto}` : "https://upload.wikimedia.org/wikipedia/en/f/f1/Tycho_-_Epoch.jpg"} alt="" />
                ...
                <div className="container px-6 py-20 mx-auto border-5 shadow-2xl">
                    ...
                    <div className="mt-5 p-8">
                        ...
                        <Form.Item label={"Mốc thời gian"}>
                            <SelectFilter onChange={(val) => { setMetric(val) }} title={"Mốc thời gian"} />
                        </Form.Item>
                    </div>
                    <ResponsiveContainer className="mt-10" width="100%" height={200}>
                        ...
                        >
                            ...
                            <XAxis dataKey="created_at" formatter={(val) => moment(val * 1000).format("DD/MM/YYYY")} />
                            <YAxis dataKey="ethPrice"/>
                            <Tooltip />
                            <Line type="monotone" dataKey="ethPrice" stroke="#8884d8" fill="#8884d8" />
                ...
                <div className="container px-6 py-20 mx-auto border-5 shadow-2xl">
                    <h1 className="text-white-900 text-3xl title-font font-medium">Comments</h1>
                    <CommentComponents nft_id={id} metamask_address={web3Ctx.account} />
                </div>

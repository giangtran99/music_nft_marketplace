...
import { request ,formatPrice} from '../../helpers/utils';
...
const Navbar = () => {
  ...
  const connectWalletHandler = async () => {
    ...
    if (result) {
      web3.eth.personal.sign(`${result.nonce}`, web3Ctx.account, '', async (err, signature) => {
        if (err) {
          toast.error(err)
          return
        }
        const authInfo = await request('/api/auth/auth-metamask', {
          publicAddress: web3Ctx.account,
          signature: signature
        }, {}, "POST")
              ...
              <div className="flex items-center justify-between h-16">
                ...
                <div className="hidden md:block">
                  <div className="ml-4 flex items-center md:ml-6">
                    ...
                    <button onClick={connectWalletHandler} className="mr-2 text-center p-auto text-center w-[120px] bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold px-3 py-2 rounded inline-flex items-center">
